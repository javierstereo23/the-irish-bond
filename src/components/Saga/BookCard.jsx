...
export default function BookCard({ book }) {
  ...
  const accent = book.colorAlt || book.color
  const badgeBg = book.colorAlt
    ? `linear-gradient(135deg, ${book.color}, ${book.colorAlt})`
    : book.color
  ...
  return (
    <div
      ...
      className="relative cursor-default select-none h-full"
      style={{
        transform: `perspective(1000px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`,
        transition: hovered ? 'transform 0.05s ease' : 'transform 0.4s ease',
      }}
    >
      {/* Glow tintado por libro */}
      <div
        className="absolute -inset-1 rounded-lg blur-xl pointer-events-none transition-opacity duration-500"
        style={{
      ...
      <div
        ...
        style={{
          background: badgeBg,
          color: book.color === '#f1e4c7' ? '#1a1a1a' : '#fff',
        }}
      >
        {book.num}
      </div>
      ...
      >
        ...
        {/* Trébol identitario — firma sutil */}
        <CloverIcon
          color={accent}
          className="absolute bottom-3 right-3 z-20 w-5 h-5 transition-opacity duration-300"
      ...
      <div className="mt-3 text-center">
        <span
          ...
          style={{
            color: book.color === '#f1e4c7' ? '#1a1a1a' : `${accent}`,
            background: book.color === '#f1e4c7' ? `${book.color}` : `${accent}15`,
